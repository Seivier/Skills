...
8. **Sintetiza y agrega la entrada del catálogo — reescribiendo el archivo completo, no editando un fragmento.** Este paso es el que hace al libro consultable: el protocolo de consulta (SKILL.md) arranca leyendo `catalog.md`, así que hasta que esta escritura no ocurre, todo lo generado en los pasos 1-7 vive en `references/library/<slug>/` pero es invisible para Estudiando/Implementando/Refactorizando/Diseñando — por diseño, para poder ingerir sin bloquear ni ensuciar la consulta de otra sesión que esté usando la biblioteca en paralelo. Lee el `book.md` completo (ya lo tienes fresco) y escribe: un resumen de 5 a 10 líneas del libro completo, y de **2 a 4 tags** seleccionados de la taxonomía estandarizada en inglés definida en la cabecera de `catalog.md` (como `software-architecture`, `design-patterns`, `code-quality-and-refactoring`, etc.). **Tageo por aporte diferencial, no por todo lo que el libro toca de pasada** — la pregunta es "¿qué tag(s) describen lo que este libro cubre mejor que ningún otro de la biblioteca?", no "¿qué temas menciona en algún capítulo?". Un libro que abarca muchos temas de refilón pero le pone 4-8 tags genéricos termina apareciendo como candidato en casi cualquier consulta del protocolo de la sección 2 de SKILL.md, ahogando a los libros de nicho que sí matchean de lleno — ya pasó con Clean Code/Clean Architecture/DDD en la ingesta inicial. Antes de cerrar la lista de tags, compárala contra las entradas ya existentes en `catalog.md`: si dos tags candidatos ya están cubiertos por otro libro con más autoridad sobre ese tema, prefiere dejar solo el tag donde este libro realmente aporta algo distinto. Lee el `catalog.md` actual, arma su contenido completo con la entrada nueva agregada al final, y escribe el archivo entero de una vez, sobrescribiéndolo por completo (no en modo append). El formato de cada entrada:
